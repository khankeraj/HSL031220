					...
					<div class="panel">
						...
						<div class="panel-body">
							<table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
								...
								<tbody>
								 <%int i=1; %>
								 <c:forEach items="${outstandingReport}" var="outstandingReport">
									<tr>
										<td><%=i++ %></td>
										<td><c:out value="${outstandingReport.issue_no}" /></td>
										<td><c:out value="${outstandingReport.date}" /></td>
										<td><c:out value="${outstandingReport.employee}" /></td>
										<td><c:out value="${outstandingReport.customer_code}" /></td>
										<td><c:out value="${outstandingReport.customer_name}" /></td>
										<td><c:out value="${outstandingReport.pono}" /></td>
										
									   
									   <td align="center"><a style="text-decoration: none;color: green;" title="Issue form" href="IssueForm?customer_no=<c:out value="${outstandingReport.customer_code}"/>&customer_name=<c:out value="${outstandingReport.customer_name}"/>&lead_id=<c:out value="${outstandingReport.lead_no}" />&pono=<c:out value="${outstandingReport.pono}"/>">
											<img alt="issue form" height="25px;" src="configure/img/mreceived_icon.png"></a>
									   </td> 
									   
									   <td align="center"><a style="text-decoration: none;color: green;" title="View" href="TaxInvoiceForm?pono=<c:out value="${outstandingReport.pono}"/>&issue_no=<c:out value="${outstandingReport.issue_no}"/>&employee=<c:out value="${outstandingReport.employee}"/>">
