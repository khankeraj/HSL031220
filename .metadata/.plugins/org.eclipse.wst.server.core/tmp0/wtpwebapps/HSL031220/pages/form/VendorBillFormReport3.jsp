...
<head>
	...
	$(function() {
		...
		// $("#advance").autocomplete("GoogleSearch/vendorbilladvanceAutoserch.jsp");
...
function getRNA(v)
{
	...
	$(".class1:checked", oTable.fnGetNodes()).each(function() {
		...
		$hoardinglocationxaa1 = $($traverse_row).find('#advance_amount');
		$hoardinglocationxaa2 = $($traverse_row).find('#finalamt');	

		//alert("advance amount is---------"+$hoardinglocationxaa1.val());
		//alert("final amount is---------"+$hoardinglocationxaa2.val());
	 
		var billamount = Number($($hoardinglocationxaa2).val());
		
		totalamount=totalamount+billamount;
		//alert("ssdasdasdsad-----"+totalamount)
		
		$finalamt = $($traverse_row).find('#finalamt').prop('readonly', true);	
	...
	//alert("final amount ffffffffffff -------"+totalamount)
	document.getElementById("total_amount1").value = totalamount;
	
	document.getElementById("vt1").value = id;
	document.getElementById("other").value = id;

	//alert("final amount -------"+totalamount)
...
function getadvance(v){
	//alert(">>>>>>>>>>>>>>>");
			...
			// alert("111112222")
			 ...
			 var billno = v.value;
			 xp1.open("POST", "getbill_amount?billbo="+ billno);
				...
				xp1.onreadystatechange = function() {
					...
					if (xp1.readyState == 4 && xp1.status == 200) {
						...
						$hoardinglocationxaa1 = $($traverse_row).find('#advance_amount');
						$hoardinglocationxaa2 = $($traverse_row).find('#finalamt');	
						//$hoardinglocationxaa2.val(price);				
						$hoardinglocationxaa1.val(price);
						//alert("00")
					

						//alert("11")
						
						 $amount = $($traverse_row).find('#total_amount');
						 
						var billamount = $($amount).val();
						
						var advamount = $($hoardinglocationxaa1).val();

						var finalamount=billamount-price;
						// alert("oooooooooo--------"+finalamount)
						$hoardinglocationxaa2.val(finalamount);
				}
				} 
...
function getadvance234() {
	

	//alert("dfddfgdfgdfdg"+v.value)
	var amount=0;

		$('.item-row').each(function(i, row) {

			var $total_amount = $(row).find('#total_amount');
			var $gstamt1 = $($total_amount).val();

			var $advance_amount = $(row).find('#advance_amount');
			var $gstamt2 = $($advance_amount).val();

			var $finalamount=$gstamt1-$gstamt2;
			
			var $advance_amount1 = $(row).find('#advance_amount1');

			//alert("final amount------"+$finalamount)
			var $gstamt3 = $($advance_amount1).val($finalamount);
...
function getAddress124(v){
	...
	if (window.XMLHttpRequest) {
	 ...
	 xp11.open("POST", "getbill_amount?billbo="+ billno);
			...
			<div id="content-container">
				...
				<div id="page-content">
					...
					<input type="text" name="vt" id="vt" >
							...
							<br>
							<br>
							<form action="venderpaymentSubmit" class="form-control " id="myformsub" method="post">
						...
						<div class="panel-body">
							<table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
								...
								<tbody>
								 ...
								 <c:forEach items="${deliveryDetails}" var="deliveryDetails">
									...
									<tr class="item-row">
										...
										<td><input  type="checkbox" class="class1" onclick="getRNA(this);" value="<c:out value="${deliveryDetails.delivery_done_id}" />"  name="dcno" id="dcno" /></td>
										
										<td>
										 <input type="text" style="background-color: #f1dfdf;"  name="billno1" id="billno1" value="<c:out value="${deliveryDetails.bill_no}" />"   required readonly="readonly">
										 <input type="text" style="background-color: #f1dfdf;"  name="bill_id" id="bill_id" value="<c:out value="${deliveryDetails.delivery_done_id}" />"   required readonly="readonly">
										</td>
                                  
										<td>
										 <input type="text" style="background-color: #f1dfdf;" name="vendorname1" id="vendorname1" value="<c:out value="${deliveryDetails.transporter_name}" />"   required readonly="readonly">
										 <input type="text" style="background-color: #f1dfdf;" name="vendorcode1" id="vendorcode1" value="<c:out value="${deliveryDetails.transporter_code}" />"   required readonly="readonly">
										</td>

											<td>
										<input type="text" style="background-color: #f1dfdf;" name="date1" id="date1" value="<c:out value="${deliveryDetails.date}" />"   required readonly="readonly"></td>
										 
									
											
											<td>
										<input type="text" style="background-color: #f1dfdf;"  name="billlamount1" id="total_amount"  value="<c:out value="${deliveryDetails.total_amount}" />" readonly="readonly"  required></td>
									
											 <script>
										$(function() {
									
											 $("#advance<%=i%>").autocomplete("GoogleSearch/vendorbilladvanceAutoserch.jsp");
											...
											<td>
										 <input type="text"  name="advance1" id="advance<%=i%>"  onblur="getadvance(this)"  ></td>
										
											<td>
										 <input type="text" style="background-color: #f1dfdf;"  name="advanceamt1" id="advance_amount" readonly="readonly" ></td>
											
										
											<td>
										 <input type="text"  name="remanamount1" id="finalamt" value="<c:out value="${deliveryDetails.total_amount}" />" required ></td>
							  ...
							  <div class="col-sm-12">
                                  ...
                                  <input type="text"  name="total_amount1" id="total_amount1"   required>
