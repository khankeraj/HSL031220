				...
				<div id="page-content">
					<div class="panel">
						<div class="panel-heading">
							<h3 class="panel-title">Trip Report</h3>
						</div>
								...
								<tbody>
								 ...
								 <c:forEach items="${tripDetails}" var="tripDetails">
									<tr>
										...
										<c:choose>
										    <c:when test="${tripDetails.up_status!='1'}">
										         
										        <a href="LR_form?trip_id=<c:out value="${tripDetails.trip_id}" />&source=<c:out value="${tripDetails.source}" />&destination=<c:out value="${tripDetails.destination}" />&vehicle_no=<c:out value="${tripDetails.vehicle_no}" />&driver_name=<c:out value="${tripDetails.driver_name}" />&updown=up"><button>Up</button></a>
										    </c:when>    
										    <c:otherwise>
										        <button disabled style="background-color: yellow;">Up</button>
										    </c:otherwise>
										</c:choose>
										
										<c:choose>
										    <c:when test="${tripDetails.down_status!='1'}">
										        <a href="LR_form?trip_id=<c:out value="${tripDetails.trip_id}" />&source=<c:out value="${tripDetails.source}" />&destination=<c:out value="${tripDetails.destination}" />&vehicle_no=<c:out value="${tripDetails.vehicle_no}" />&driver_name=<c:out value="${tripDetails.driver_name}" />&updown=down"><button>Down</button></a>
										    </c:when>    
										    <c:otherwise>
										        <button disabled style="background-color: yellow;">Down</button>
										    </c:otherwise>
										</c:choose>
										
										<td>
											<%-- <a href="trip_issue_form1?trip_id=<c:out value="${tripDetails.trip_id}" />&source=<c:out value="${tripDetails.source}" />&destination=<c:out value="${tripDetails.destination}" />&vehicle_no=<c:out value="${tripDetails.vehicle_no}" />&driver_name=<c:out value="${tripDetails.driver_name}" />"><button>Trip issue</button></a> --%> 
											<a href="trip_issue_form1?trip_id=<c:out value="${tripDetails.trip_id}" />&source=<c:out value="${tripDetails.source}" />&destination=<c:out value="${tripDetails.destination}" />&vehicle_no=<c:out value="${tripDetails.vehicle_no}" />&driver_name=<c:out value="${tripDetails.driver_name}" />"><img alt="" height="20px" width="30px" src="configure/img/budget.png"></a>
										</td>
										
										
										
										
										<%-- <c:if test = "${tripDetails.up_status!='1'}">
											<a href="LR_form?trip_id=<c:out value="${tripDetails.trip_id}" />&source=<c:out value="${tripDetails.source}" />&destination=<c:out value="${tripDetails.destination}" />&vehicle_no=<c:out value="${tripDetails.vehicle_no}" />&driver_name=<c:out value="${tripDetails.driver_name}" />&updown=up"><button>Up</button></a>
										 </c:if>
										
										<c:if test = "${tripDetails.down_status!='1'}">
											<a href="LR_form?trip_id=<c:out value="${tripDetails.trip_id}" />&source=<c:out value="${tripDetails.source}" />&destination=<c:out value="${tripDetails.destination}" />&vehicle_no=<c:out value="${tripDetails.vehicle_no}" />&driver_name=<c:out value="${tripDetails.driver_name}" />&updown=down"><button>Down</button></a>
										</c:if> --%>	
										</td>
