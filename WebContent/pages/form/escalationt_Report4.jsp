...
<script type="text/javascript" language="javascript" class="init">
  ...
  <!-- AutoSearch -->
	...
	$(function() {
		...
		// $("#driver_name").autocomplete("GoogleSearch/autosearchDriver.jsp");
		 ...
		 $("#dealer_id").autocomplete("GoogleSearch/transporterautosearch.jsp");
 ...
 <%
		 ...
		 long order=0;
		 long order_approve=0;
		 long vehicle_assign=0;
		 long loading_done=0;
		 long bill=0;
		 long bill_approve=0;
		 long delivery=0;
		 long delivery_done=0;
     ...
     try
     {
 		...
 		.prepareStatement("select *  from escalition_time");
 		...
 		if (resultSet12.next()) {
 			...
 			//order = resultSet12.getString("order") ;
 			order_approve = resultSet12.getLong("pricing") ;
 			System.out.println("-------------"+resultSet12.getLong("pricing"));
 			vehicle_assign = resultSet12.getLong("quotation") ;
 			loading_done = resultSet12.getLong("finalize") ;
 			bill = resultSet12.getLong("vehicalconf") ;
 			bill_approve = resultSet12.getLong("creatlr") ;
 			delivery = resultSet12.getLong("deliverydone") ;
 			delivery_done = resultSet12.getLong("invoice") ;
				...
				<div id="page-content">
					<div class="panel">
						<div class="panel-heading">
							<h3 class="panel-title"> Escalation Report</h3>
						</div>
						<div class="panel-body">
						<form action="escalation_Report_Submitv">
							...
							<table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
								<thead>
									<tr>
										...
										<th>Vehicle Conf Date</th>
										<th>Create Lr Date</th>
										
										<th>Delivery Done Date</th>
							...
							<tbody>
												...
												<c:set var="number1" value="<%=order_approve %>"/>
												<c:set var="number2" value="<%=vehicle_assign %>"/>
												<c:set var="number3" value="<%=loading_done %>"/>
												<c:set var="number4" value="<%=bill %>"/>
												<c:set var="number5" value="<%=bill_approve %>"/>
												<c:set var="number6" value="<%=delivery %>"/>
												<c:set var="number7" value="<%=delivery_done %>"/>	
								...
								<%-- <s:iterator value="tripDetails"> --%>
								 <c:forEach items="${tripDetails}" var="tripDetails">
									<tr>
										...
										<td> <c:out value="${tripDetails.emp_id}" /></td>
										
										<td> <c:out value="${tripDetails.dc_date}" /></td>
										
										
										 <c:choose>
											    <c:when test="${tripDetails.t1 >number1}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.emp_name}" /></td>
											        
											    </c:when>    
											    <c:otherwise>
											       <td><c:out value="${tripDetails.emp_name}" /></td> 
											        
											    </c:otherwise>
											</c:choose> 
											<c:choose>
											    <c:when test="${tripDetails.t2 >number2}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.documentsno}" /></td>
											        
											    </c:when>    
											    <c:otherwise>
											       <td><c:out value="${tripDetails.documentsno}" /></td> 
											        
											    </c:otherwise>
											</c:choose> 
											
										<c:choose>
											    <c:when test="${tripDetails.t3 >number3}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.aa1}" /></td>
										...
										<c:choose>
											    <c:when test="${tripDetails.t4 >number4}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.type}" /></td>
											        
											    </c:when>    
											    <c:otherwise>
											       <td><c:out value="${tripDetails.type}" /></td> 
											        
											    </c:otherwise>
											</c:choose> 
										
										<c:choose>
											    <c:when test="${tripDetails.t5 >number5}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.remark}" /></td>
											        
											    </c:when>    
											    <c:otherwise>
											       <td><c:out value="${tripDetails.remark}" /></td> 
											        
											    </c:otherwise>
											</c:choose> 
										
										<c:choose>
											    <c:when test="${tripDetails.t6 >number6}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.add2}" /></td>
											        
											    </c:when>    
											    <c:otherwise>
											       <td><c:out value="${tripDetails.add2}" /></td> 
										...
										<c:choose>
											    <c:when test="${tripDetails.t7 >number7}">	
											      <td style="background-color: orange"><c:out value="${tripDetails.cash_source}" /></td>
											        
											    </c:when>    
											    <c:otherwise>
											       <td><c:out value="${tripDetails.cash_source}" /></td> 
											        
											    </c:otherwise>
											</c:choose> 
										<%-- 	
										<td><s:property value="emp_name" /></td>
										<td><s:property value="documentsno"/></td> 
										
										<td><s:property value="aa1"/></td> 
										...
										<td><s:property value="cash_source" /></td> --%>
