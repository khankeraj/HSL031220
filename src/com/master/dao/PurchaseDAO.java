...
public class PurchaseDAO {
	...
	public String insertCustomerInfo(invoicebean ib,String invoice_no,String lead_no) throws SQLException,ParseException {
		String response="";
...
//										"insert into purchase_order(pono,date,customer_name,supplier_name,job_name,status,done_status,dc_status,username,datetime) values(?,?,?,?,?,?,?,?,?,?) ");
//
//								pst.setString(1, pb.getJob_card_id());
//								pst.setString(2, CoreData.getDateFormatAsDb(pb.getDate()));
//								pst.setString(3, pb.getClient_name());
//
//								pst.setString(4, pb.getAddress());
//								pst.setString(5, pb.getJob_name());
//								pst.setString(6, "0");
//								pst.setString(7, "0");
//								pst.setString(8, "0");
//								
//								pst.setString(9, bean.getUsername());
//								pst.setString(10, SystemDateTime.CurrentDateTime());
//
//								int i = pst.executeUpdate();

								for (int d = 0; d < ib.getDescription1().length; d++) {

									PreparedStatement pst2 = con.prepareStatement(
											"insert into purchase_order_details(pono,descrip,quantity,remaining_quantity) values(?,?,?,?) ");

									pst2.setString(1, ib.getInvoiceno());
									pst2.setString(2, ib.getDescription1()[d]);
									pst2.setString(3, ib.getQuantity1()[d]);
									pst2.setString(4, ib.getQuantity1()[d]);
...
//									pst2.setString(6, SystemDateTime.CurrentDateTime());
							...
							PreparedStatement preparedStatement = con
									.prepareStatement("select * from purchase where  invoice_no=?");
				...
				query = "UPDATE   purchase  set vehicle_no=?,date=?,job_card_no=? ,invoice_done_status='0',total=?,pono=?,podate=?,termcond=?,vendor=?,dcno=?,dcdate=?,transmode=?,contactp=?,contactpno=?,shipparty=?,shipadd=?,shipgstn=?,shipstate=?,vehino=?,freight=?,transport=?,cgstamount=?,sgstamount=?,totaltax=?,remaining_amount=? where invoice_no=?";
...
/*
			...
			*/
			...
			int i = 0;
			int l1 = 0;
			String q1 = "insert into purchase_tax_details(invoice_no,type,descrip,qty,amt,vat_percent,tax_amt_percent,net_amt,type_name,cgstrate,cgstamount,sgstrate,sgstamount,rate,partno,partdate,partnox,disc,discamt,amtwithdisc,perunitqty) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			for (int j1 = 0; j1 < ib.getAmount1().length; j1++) {
				...
				int n12 = pstdel1.executeUpdate();
					...
					query = "insert into   purchase_hsn_details  (invoiceno,hsncode,cgstrate,sgstrate,cgstamount,sgstamount,rate, taxablevalue) values (?,?,?,?,?,?,?,?) ";
			...
			int ntc = pstdeltc.executeUpdate();
			 q1 = "insert into purchase_tax_collection(invoice_no,tax_type,taxable_amt,tax_amt,date) values(?,?,?,?,?)";
			...
			for (int j1 = 0; j1 < ib.getT().length; j1++) {
				...
				query = "UPDATE   purchase_customercreditdebit  set Amount=? where Documentsno=?";
			...
			System.out.println("in purchase dao");
			
			String query1="INSERT INTO purchase(vehicle_no,date,pay_mode,paid_amount,balance_amount,CHECK_date,check_no,card_trn_id,insurance_comp,invoice_no,paybycash,paybycheque,paybycredit,job_card_no,total,bankname,insurance_date,invoice_done_status,pono,podate,termcond,vendor,dcno,dcdate,transmode,contactp,contactpno,shipparty,shipadd,shipgstn,shipstate,vehino,freight,transport,invtype,remaining_amount,cgstamount,sgstamount,totaltax,lead_no,status)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)";
			...
			PreparedStatement pst1=con.prepareStatement("UPDATE `status_master` SET `suppo`='YES' WHERE lead_no='"+lead_no+"'");
			int jk=pst1.executeUpdate();
			
			System.out.println("update status master:"+pst1+"jk:"+jk);
			
			 PreparedStatement pst=con.prepareStatement("UPDATE `customerpayment` SET `status`=2 WHERE `Invoiceno`='"+invoice_no+"' AND lead_no='"+lead_no+"'");
			
			 int i=pst.executeUpdate();
			 
			 
			 System.out.println("update pi>>>>>>>>>>>>>>>>>>>>>>>>>>:"+pst+"i:"+i);
			
			// Insert Into Customer Credit Debit Report
			 
			 
	
			String query11="insert into purchase_customercreditdebit(Customercode,Date,Documentsno,Remark,Typecode,type,Amount,Name)VALUES(?,?,?,?,?,?,?,?)";
...
*/
 ...
 System.out.println("Sql12::"+ps1);
	...
	if (resultSet.next()) {
		...
		System.out.println("PURCHASE IN DAO:"+query1);
		pst9=con.prepareStatement(query1);
		pst9.setString(1, ib.getCustomer_Id());
		pst9.setString(10,stringValue1);
		pst9.setString(14, ib.getJob_Card_no());
		...
		if(flag1==0)
		{
			...
			if (resultSet.next()) {
				...
				if (ib.getPaymod()==null  || ib.getPaymod().equals("")) {	
					...
					query = "UPDATE   purchase  set vehicle_no=?,date=?,job_card_no=? ,invoice_done_status='0',total=?,pono=?,podate=?,termcond=?,vendor=?,dcno=?,dcdate=?,transmode=?,contactp=?,contactpno=?,shipparty=?,shipadd=?,shipgstn=?,shipstate=?,vehino=?,freight=?,transport=?,igstamount=?,totaltax=?,remaining_amount=? where invoice_no=?";
...
*/				
	...
	pst9.setString(11,"0");
	 ...
	 System.out.println("Sql12::"+ps1);
		...
		public List<invoicebean> purchasereport(List<invoicebean> invoicebean) {
			// TODO Auto-generated method stub
			...
			try {
//				PreparedStatement pst=conn.prepareStatement("SELECT purchase.invoice_no,purchase.vehicle_no,customer_master_details.customer_name,purchase.date,purchase.total FROM customer_master_details,purchase WHERE customer_master_details.customer_no=purchase.vehicle_no");
				PreparedStatement pst=conn.prepareStatement("SELECT purchase.invoice_no,purchase.vehicle_no,customer_master_details.customer_name,purchase.date,purchase.total,purchase.vendor FROM customer_master_details,purchase WHERE customer_master_details.customer_no=purchase.vehicle_no and purchase.status=1");
				
				System.out.println("pucrhaseDetails:"+pst);
				ResultSet rs=pst.executeQuery();
				while(rs.next())
				{
					invoicebean ib=new invoicebean();
					ib.setPo_number(rs.getString(1));
					ib.setCustomer_no(rs.getString(2));
					ib.setCustomer_name(rs.getString(3));
					ib.setDate(rs.getString(4));
					ib.setTotal(rs.getString(5));
					ib.setVendor(rs.getString(6));
					
					int flag=0;
					
					PreparedStatement pst01 = conn.prepareStatement("select remaining_quantity from purchase_order_details where pono ='"+rs.getString(1)+"'");
					ResultSet rs1 = pst01.executeQuery();
					
					
					while(rs1.next())
					{
						if(Double.parseDouble(rs1.getString(1))>0) {
							flag=1;
							break;
						}
					}
		...
		public List<invoicebean> fetchForUpdatePurchaseOrder(List<invoicebean> purchase_view_details,String purchase_po_no,String customer_code,String date) {
			// TODO Auto-generated method stub
			List<invoicebean> list=new ArrayList<>();
			...
			try {
				...
				PreparedStatement pst=conn.prepareStatement("SELECT `customer_no`, `customer_name`,`customer_address`, `mobile_no`, `pending_amt`, `phone_no`, `state`, `city`,`pan_no`, `customer_gst_no` FROM `customer_master_details` WHERE customer_master_details.customer_no='"+customer_code+"'");
				ResultSet rs=pst.executeQuery();
				...
				while(rs.next())
				{
					...
					ib.setPo_number(purchase_po_no);
					ib.setDate(date);
					ib.setCustomer_no(rs.getString(1));
					ib.setCustomer_name(rs.getString(2));
					ib.setAddress(rs.getString(3));
					ib.setMobile(rs.getString(4));
					ib.setBalance_amt(rs.getString(5));
					ib.setContactp(rs.getString(6));
					ib.setState(rs.getString(7));
					ib.setCity(rs.getString(8));
					ib.setPan_no(rs.getString(9));
					ib.setCustomer_gstin_no(rs.getString(10));
					list.add(ib);
				}
