...
public class Resource_Type_Master_Dao {
	
	
	static DBConnection connection=new DBConnection();
	//static Connection conn=connection.getConnection();

	public String insertResourceTypeMaster(TransportBean bin, userinfo bean)throws SQLException,ParseException {
		...
		try {
					...
					PreparedStatement pst = conn.prepareStatement("select *  from inquiry_resource where resource=? ");
				...
				//	pst.setString(2,bean.getClinicId());
					...
					
					{
						...
						PreparedStatement preparedStatement11 = conn
								.prepareStatement("select  max(SUBSTRING(resource_id,-5)) as myval1 from inquiry_resource  ");
					...
					PreparedStatement pst2 = conn.prepareStatement(
							"insert into inquiry_resource(resource_id,resource,username,datetime) values(?,?,?,?) ");
	...
	public List<TransportBean> displayResourceTypeMaster(userinfo bean) {
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("select *  from inquiry_resource  ");
		...
		
			{
				...
				bean1.setModel(resultSet1.getString("resource"));
	...
	public List<TransportBean> displayResourceTypeMasterEdit(userinfo bean,String id) {
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("select *  from inquiry_resource where id="+id+" ");
	...
	public static List<TransportBean> updateResourceTypeMaster(TransportBean pb,userinfo bean) throws Exception {
		...
		try{
			System.out.println("SQL:");
					...
					String q1 = "update inquiry_resource set resource='"+pb.getModel()+"',"
	...
	public static String deleteResourceTypeMaster(TransportBean pb,userinfo bean,String id) throws Exception {
		
		Connection conn=connection.getConnection();

		try {
			
			PreparedStatement pst4 = conn.prepareStatement("delete from inquiry_resource where id='"
							+ id+ "' ");
