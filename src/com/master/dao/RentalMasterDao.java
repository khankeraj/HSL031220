...
public class RentalMasterDao {
	...
	public int update_rental_details(RentalModel rental_model, String update_rental) {
		// TODO Auto-generated method stub
		int i=0;
		int rental_id=Integer.parseInt(update_rental);
		
		try {
			PreparedStatement pst=conn.prepareStatement("UPDATE `rental_master` SET `rental_name`=?,`rental_description`=? WHERE `id`=?");
			
			pst.setString(1, rental_model.getRental_name());
			pst.setString(2, rental_model.getRental_description());
