...
public class Vehicle_Type_Master_Dao {
	
	
	static DBConnection connection=new DBConnection();
	//static Connection conn=connection.getConnection();

	public String insertVehicleTypeMaster(TransportBean bin, userinfo bean)throws SQLException,ParseException {
		...
		try {
					...
					PreparedStatement pst = conn.prepareStatement("select *  from vehicle_type_master where model=? ");
				...
				//	pst.setString(2,bean.getClinicId());
					...
					
					{
						...
						PreparedStatement preparedStatement11 = conn
								.prepareStatement("select  max(SUBSTRING(vehicle_id,-5)) as myval1 from vehicle_type_master  ");
					...
					PreparedStatement pst2 = conn.prepareStatement(
							"insert into vehicle_type_master(vehicle_id,model,size,no_of_bag,no_of_box,username,datetime,halt_charge) values(?,?,?,?,?,?,?,?) ");
					...
					pst2.setString(8,bin.getHalt_charge());
	...
	public List<TransportBean> displayVehicleTypeMaster(userinfo bean) {
		...
		try {
		...
		
			{
				...
				bean1.setModel(resultSet1.getString("model"));
				...
				bean1.setHalt_charge(resultSet1.getString("halt_charge"));
	...
	public List<TransportBean> displayVehicleTypeMasterEdit(userinfo bean,String id) {
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("select *  from vehicle_type_master where id="+id+" ");
	...
	public static List<TransportBean> updateVehicleTypeMaster(TransportBean pb,userinfo bean) throws Exception {
		...
		try{
			System.out.println("SQL:");
					...
					String q1 = "update vehicle_type_master set model='"+pb.getModel()+"',"
							+ "username='"+bean.getUsername()+"',"
							+ "datetime='"+SystemDateTime.CurrentDateTime()+"',"
							+ "size='"+pb.getSize()+"',"
							+ "halt_charge='"+pb.getHalt_charge()+"',"
							+ "no_of_bag='"+pb.getNo_of_bag()+"',"
							+ "no_of_box='"+pb.getNo_of_box()+"' where id='"+pb.getId()+"'";
	...
	public static String deleteVehicleTypeMaster(TransportBean pb,userinfo bean,String id) throws Exception {
		
		Connection conn=connection.getConnection();

		try {
			
			PreparedStatement pst4 = conn.prepareStatement("delete from vehicle_type_master where id='"
							+ id+ "' ");
